...
Descrição:
    Este script análise os eventos 4727 (A security-enabled global group was created), 4731 (A security-enabled local group was created) e 4754 (A security-enabled universal group was created) no log de Security de todos os servidores Domain Controllers do domínio a partir de um período de tempo especifico.
    Útil para manter guardado registro de eventos de auditoria e identificar de maneira rápida o administrador que criou um grupo de segurança no domínio.
...
Exemplos:
    ...
    C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -file C:\Script1\AuditCreateGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SenderTitle "Auditoria AD" -SmtpServer 10.1.1.1

    Este exemplo pode ser executado da console do Windows PowerShell. Ele captura eventos registrados até uma hora passada.
    PS > C:\Script1\AuditCreateGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -SenderTitle "Auditoria AD" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SmtpServer 10.1.1.1
------------------------------------------------------------------------
#>
...
$MessageSubject = "Audit AD Group Create " + (Get-Date -Format F)
$LineDCName = "#212C4E"
$LineClear = "#DBEAFF"
$LineDark = "#BCD0EC"
...
foreach ($DCLine in $AllDC)
    {
        ...
        Try
        {
            $LogDc = Get-WinEvent -LogName Security -ComputerName $DcFinal -FilterXPath "*[System[(EventID=4727 or EventID=4731 or EventID=4754) and Task=13826 and TimeCreated[timediff(@SystemTime) < $CriterionMinutes]]]" -ErrorAction Stop
        }
