...
Descrição:
    Este script análise os eventos de gerenciamento de membros de grupos de segurança do Active Directory no log de Security de todos os servidores Domain Controllers do domínio a partir de um período de tempo especifico.
    Os seguintes Id de eventos são analisados:
    ...
    Útil para manter guardado registro de eventos de auditoria e identificar de maneira rápida o administrador que alterou o membro de algum grupo de segurança.
...
Exemplos:
    ...
    C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -file C:\Script1\AuditGroupMember.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SenderTitle "Auditoria AD" -SmtpServer 10.1.1.1

    Este exemplo pode ser executado da console do Windows PowerShell. Ele captura eventos registrados até uma hora passada.
    PS > C:\Script1\AuditGroupMember.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -SenderTitle "Auditoria AD" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SmtpServer 10.1.1.1
------------------------------------------------------------------------
#>
...
$MessageSubject = "Audit AD Group Member " + (Get-Date -Format F)
$LineDCName = "#3B5737"
$LineClear = "#C0E4BA"
$LineDark = "#74D166"
...
foreach ($DCLine in $AllDC)
    {
        ...
        Try
        {
            $LogDc = Get-WinEvent -LogName Security -ComputerName $DcFinal -FilterXPath "*[System[(EventID=4728 or EventID=4729 or EventID=4732 or EventID=4733 or EventID=4756 or EventID=4757) and Task=13826 and TimeCreated[timediff(@SystemTime) < $CriterionMinutes]]]" -ErrorAction Stop
        }
