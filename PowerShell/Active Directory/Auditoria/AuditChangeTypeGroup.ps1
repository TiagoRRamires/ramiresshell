...
Descrição:
    Este script analise o evento 4764 no log de Security de todos os servidores Domain Controllers do domínio a partir de um período de tempo especifico.
    Útil para manter guardado registro de eventos de auditoria e identificar de maneira rápida o administrador que alterou o tipo de um grupo de segurança para distribuição ou vice/versa. Identifica também alterações no escopo do grupo (Universal, Global e Local).
...
Exemplos:
    ...
    C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -file C:\Script1\AuditChangeTypeGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SenderTitle "Auditoria AD" -SmtpServer 10.1.1.1

    Este exemplo pode ser executado da console do Windows PowerShell. Ele captura eventos registrados até uma hora passada.
    PS > C:\Script1\AuditChangeTypeGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -SenderTitle "Auditoria AD" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SmtpServer 10.1.1.1
------------------------------------------------------------------------
#>
...
$MessageSubject = "Audit AD ChangeType Group " + (Get-Date -Format F)
$LineDCName = "#995116"
$LineClear = "#FAD3B4"
$LineDark = "#FDA55F"
...
foreach ($DCLine in $AllDC)
    {
        ...
        Try
        {
            $LogDc = Get-WinEvent -LogName Security -ComputerName $DcFinal -FilterXPath "*[System[EventID=4764 and Task=13826 and TimeCreated[timediff(@SystemTime) < $CriterionMinutes]]]" -ErrorAction Stop
        }
