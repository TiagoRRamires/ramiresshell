...
Descrição:
    Este script análisa os eventos 4730 (A security-enabled global group was deleted), 4734 (A security-enabled local group was deleted) e 4758 (A security-enabled universal group was deleted) no log de Security de todos os servidores Domain Controllers do domínio a partir de um período de tempo especifico.
    Útil para manter guardado registro de eventos de auditoria e identificar de maneira rápida o administrador que excluiu um grupo de segurança do Active Directory.
...
Exemplos:
    ...
    C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe -file C:\Script1\AuditRemoveGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SenderTitle "Auditoria AD" -SmtpServer 10.1.1.1

    Este exemplo pode ser executado da console do Windows PowerShell. Ele captura eventos registrados até uma hora passada.
    PS > C:\Script1\AuditRemoveGroup.ps1 -DNDomain "DC=ramiresshell,DC=lab" -CriterionMinutes 60 -Sender "no-reply@ramiresshell.com.br" -SenderTitle "Auditoria AD" -Recipients "tiago@ramiresshell.com.br", "ramirestiago@hotmail.com" -SmtpServer 10.1.1.1
------------------------------------------------------------------------
#>
...
$MessageSubject = "Audit AD Group Remove " + (Get-Date -Format F)
$LineDCName = "#5B0101"
$LineClear = "#F3A3A3"
$LineDark = "#E43A3A"
...
foreach ($DCLine in $AllDC)
    {
        ...
        Try
        {
            $LogDc = Get-WinEvent -LogName Security -ComputerName $DcFinal -FilterXPath "*[System[(EventID=4730 or EventID=4734 or EventID=4758) and Task=13826 and TimeCreated[timediff(@SystemTime) < $CriterionMinutes]]]" -ErrorAction Stop
        }
