...
Descrição:
    ...
    Útil para copia de grande volume de arquivos entre diretórios ou servidores.
Considerações:
...
Exemplos:
    Este exemplo pode ser executado em schedule ou a partir do prompt de comandos. Copia arquivos da pasta C:\Arquivos1 do computador onde o script esta sendo executado para o caminho de rede \\FileServer\Arquivo\Folder1. Sobrepõem arquivos já existentes no destino. Executa 3 tentativas de copias para arquivos que estiverem apresentando problemas na copia. Aguarda 40 segundos entre as tentativas de copia que estão apresentando problemas.
    ...
    Este exemplo pode ser executado da console do Windows PowerShell. Copia arquivos do caminho de rede \\FS2\Archieve\1 para o caminho de rede \\FileServer\Arquivo\Folder1. Não sobrepõem arquivos já existentes no destino. Executa 3 tentativas de copias para arquivos que estiverem apresentando problemas na copia. Aguarda 2 segundos entre as tentativas de copia que estão apresentando problemas.
            ...
            ForEach ($SourceFile in $SourceFileTable)
            {
                ...
                Else
                {
                    While ($RetryCopy -le $Retry)
                    {
                        ...
                        {
                            Start-Sleep -Seconds $Wait
                            Add-Content -LiteralPath $FileLog -Value ((Get-Date).ToString("dd/MM/yyyy HH:mm:ss") + " Job: $MyId  - Tentativa n.º $RetryCopy para copia do arquivo: $SourceFile") -Force -Encoding Unicode
                            $RetryCopy += 1
                        }
                    }
                    ...
                    If (Test-Path -LiteralPath $FileDestination)
                    {
                        ...
                        Else
                        {
                            If ($CopyError)
                            {
                                Add-Content -LiteralPath $FileLog -Value ((Get-Date).ToString("dd/MM/yyyy HH:mm:ss") + " Job: $MyId  - Erro na copia - Já existe no destino com tamanho DIFERENTE - Origem: $SourceFile  Tamanho: $SourceFileLen  -->>  destino: $FileDestination  Tamanho: $FileDestinationLen  "  + $Error[0].Exception.Message + "   " + $Error[0].InvocationInfo.InvocationName) -Force -Encoding Unicode
                                $FileError += 1
                            }
                            Else
                            {
                    ...
                    Else
                    {
                        Add-Content -LiteralPath $FileLog -Value ((Get-Date).ToString("dd/MM/yyyy HH:mm:ss") + " Job: $MyId  - Copia NÃO executada - Origem: $SourceFile  -->>  destino: $FileDestination") -Force -Encoding Unicode
                        $FileNotCopy += 1
                    }
                }
                $MyId += 1
...
Add-Content -LiteralPath $FileLog -Value ("$Block    Diretórios -   Criados: $DirCreated     Existentes: $DirExisting     Não encontrados: $DirNotFound") -Force -Encoding Unicode
Add-Content -LiteralPath $FileLog -Value ("$Block    Arquivos   -   Copiados: $FileCopied     Tamanhos Diferentes: $FileDiffSize     Não Copiado: $FileNotCopy     Erros: $FileError") -Force -Encoding Unicode
