...
Descrição:
    ...
    Pode se definir um valor máximo de threads para copias simultâneas de arquivos. Utiliza RunSpaces do .Net Framework, que consome menos memória e possui melhor performance comparado ao jobs do Windows PowerShell.
    ...
    Útil para copia de grande volume de arquivos entre diretórios ou servidores, com ganho de performance com copias simultâneas.
Considerações:
    ...
    Não use no Windows PowerShell v4, esta versão apresenta problemas de memory leak com o RunSpace.
...
#Valida versão do Windows PowerShell
If ((Get-Host).Version.Major -lt 3)
{
    Add-Content -LiteralPath $FileLog -Value ((Get-Date).ToString("dd/MM/yyyy HH:mm:ss") + " Versão não comporta todos cmdlet utilizados pelo script. Recomendado versão 5.1 do Windows PowerShell") -Force -Encoding Unicode
    Exit
}

If ((Get-Host).Version.Major -eq 4)
{
    Add-Content -LiteralPath $FileLog -Value ((Get-Date).ToString("dd/MM/yyyy HH:mm:ss") + " Windows PowerShell v4 apresenta erros de memory leak com runspace e jobs. Recomendado versão 5.1 do Windows PowerShell") -Force -Encoding Unicode
    Exit
}
